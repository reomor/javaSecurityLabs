...
7. [Реализация механизмов контроля доступа (Небезопасные прямые ссылки на объекты)](#idor)
8. [Обработка запросов на изменение данных (защита от CSRF)](#csrf)
9. [Небезопасные настройки компонентов](#misconfig)
...
2. Запустите модуль. Зайдите на http://localhost:8091/
3. Залогиньтесь под пользователем _user_ с паролем _password_
